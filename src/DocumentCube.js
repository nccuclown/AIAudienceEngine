...
export const DocumentCube = ({ showCube, documentParticles, progress }) => {
  ...
  console.log("DocumentCube渲染:", {
    showCube,
    particles: documentParticles.length,
    progress,
  });

  // 如果為false仍然返回一些基本內容，而不是完全不渲染
  if (!showCube) {
    console.log("DocumentCube未顯示");
    return (
      <div className="cube-container" style={{ opacity: 0 }}>
        {/* 即使不顯示也保持DOM結構 */}
      </div>
    );
  }
    ...
    <div className="cube-container">
      ...
      {selectedDoc && selectedDoc.hasContent && !selectedDoc.isFlying && (
        <div
          ...
          style={{
            left: `${selectedDoc.displayX}%`,
            top: `${selectedDoc.displayY - 15}%`,
